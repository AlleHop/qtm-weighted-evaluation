...
for root, dirs, f in os.walk(path):
    ...
    for dir in dirs:
        ...
        if(len(filenames) == 0):
            continue
        output_name = '_'.join(filenames[0].split('_')[1:-1]) + '_all_nochanges.csv'
        seeds = []
        ...
        df['mink'] = df.groupby(['seed', 'initialization', 'graph','subtreeMove','subtreeSortPaths'])['editCosts'].transform(np.min)
        df = df[df.mink == df.editCosts].groupby(['seed', 'initialization', 'graph','subtreeMove','subtreeSortPaths']).first().reset_index()

    ...
    for graph in $graph_list; do
      for seed in $seeds; do
        if [ $scenario == "weighted" ]; then
          python3 python_scripts/evaluation_weighted.py -g ${graph}".graph" -p "../output/QTM_matrix/${graph_set}/temp_${scenario}/" -s ${scenario} -r ${seed} &
        else
          python3 python_scripts/evaluation_weighted.py -g ${graph}".graph" -w ${graph}".csv" -p "../output/QTM_matrix/${graph_set}/temp_${scenario}/" -s ${scenario} -r ${seed} &
        fi  
      done
      wait
    done
    python3 python_scripts/means.py -p "../output/QTM_matrix/${graph_set}/temp_${scenario}/"
  done
done
